...
本章介绍 高级字符设备驱动特性。涵盖的核心知识点包括：异步 I/O、内存映射、设备控制。

## 知识点详解

### 1. 核心概念

高级字符设备驱动特性是 Linux 驱动开发中的重要组成部分。本章通过实际代码示例，展示了如何使用 Linux 内核提供的相关 API 进行驱动开发。

**关键 API**：poll/select、mmap、unlocked_ioctl
...
┌─────────────────────────────────────────────────────────────┐
│                    应用程序 (User Space)                      │
│  read() / write() / poll() / ioctl() / fcntl(O_ASYNC)        │
└────────────────────┬────────────────────────────────────────┘
                     │ 系统调用
┌────────────────────▼────────────────────────────────────────┐
│                  VFS 文件系统层                               │
